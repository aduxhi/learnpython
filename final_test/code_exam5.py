...
class courseInfo(object):
    ...
    def __init__(self, courseName):    #特殊方法
        self.courseName = courseName   
        self.psetsDone = []
        self.grade = "No Grade"
        
    def setPset(self, pset, score):     #方法
...
class edx(object):
    def __init__(self, courses):
        self.myCourses = []         #在方法中对实例变量进行赋值初始化。实例变量了，myCourses

        for course in courses:
            self.myCourses.append(courseInfo(course)) # 实例化



    def setGrade(self, grade, course="6.01x"):  #course 为默认参数
        """
    ...
    def setPset(self, pset, score, course="6.00x"):
        ...
        If `course` is not part of the initialization, then no pset score 
        is set, and no error is thrown.
        """
    ...
    def getPset(self, pset, course="6.00x"): #关键字参数
        """
        pset: a string or a number
        course: string        
